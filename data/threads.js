(async () => {
  ...
  if (cmd) {
    ...
    if (threads.length) {
      ...
      const ik = /var GLOBALS=\[(?:([^,]*),){10}/.exec(document.body.innerHTML);
      if (ik) {
        let n = 0;
        for (const thread of threads) {
          ...
          await new Promise(resolve => {
            ...
            const src = `https://mail.google.com/mail/u/${num}?ik=${ik[1]}&view=pt&search=all` +
        ...
        alert('Cannot detect the "ik" from this view. Please open the thread and use the print buttons');
      }
    }
    else {
